...
### Status Updates
- FR-005: Office can see lesson status (pending / confirmed / rejected) refresh automatically at a regular interval without manual page reload. Priority: must-have
  > Socratic: Counter-argument considered: "push-based real-time sync is the hardest technical piece — polling every 30 seconds covers the same UX need." Resolution: accepted. FR-005 downgraded from real-time push to polling. Push-based real-time sync moves to nice-to-have / v2. For a small school with low concurrent activity, polling is indistinguishable in practice.
...
- The instructor view is usable on a mobile browser without horizontal scrolling or pinch-zoom — instructors open their unique URL on a phone.
...
**Office role** — one shared account (email + password). Full access: view instructor calendars, create lessons, link students to lessons, filter by licence category. Instructor and student profiles are pre-seeded; no in-app profile management in MVP.

**Instructor role** — no login. Each instructor has a unique URL token. Token gates access to that instructor's view only. Capabilities: see their own pending and confirmed lessons, approve or reject a pending lesson, see the student name attached to a lesson. Cannot view other instructors' calendars or any school-wide data.

**Unauthenticated** — no access. Any request without a valid session (office) or a valid instructor token is rejected.
...
- No AI-suggested scheduling — no automatic matching of student availability to instructor slots; marked as a future iteration.
- No payments or invoices — no financial features of any kind in MVP.
- No real-time push — status updates use polling; push-based real-time sync is v2.
- No mobile app — web only; the instructor view is responsive but not a native mobile application.

## Open Questions

No open questions. Input scored 4/4 on the shape heuristic; quality_check_status: accepted on 2026-05-19. All domain decisions resolved during shaping.
