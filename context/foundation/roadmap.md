...
1. **What exact licence categories does the school use (B, C, D, T, B+E, C+E…), and is the complete list finalized with the client?** — Owner: user. Block: F-01 (seed data uses placeholder categories until this is confirmed; does not block development, only the final seeder content). Also blocks **TD-01** (`lesson-category-invariant`): the student↔lesson category rule should be enforced against the finalized category set, so confirm categories before hardening the invariant.
