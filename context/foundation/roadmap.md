...
| TD-07      | past-slot-click-feedback | Show feedback when clicking a disabled past-time slot in the office calendar | yes | 🟡 Flagged by user (2026-09-04) as a follow-up to TD-06: clicking a disabled past slot in `CalendarGrid` is currently a silent no-op (deliberate scope decision in TD-06 — the user explicitly asked for a click-level block, not an error message). Add a toast/tooltip explaining the slot can't be booked, so the lack of response doesn't read as the app being unresponsive. GitHub issue #71. |
