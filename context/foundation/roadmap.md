...
| TD-03      | shadcn-design-refresh    | Align UI with shadcn/ui blocks + wire up dark mode                        | done — merged (PR #65); one follow-up fix branch (`fix/shadcn-reject-chip-overflow-v2`, mobile viewport overflow found by user testing) still pending merge | 🟡 Design-system debt: shadcn/ui is installed and configured (`components.json`, full light/dark token set in `globals.css`) but only 6 of 67 primitives are pulled in, and 3 of those (`Badge`, `Drawer`, `Popover`) are installed and never imported — every real screen (`/login`, `/office`, `/lesson/[token]`) hand-rolls Tailwind div soup for things the design system already solves. Dark mode has complete `:root`/`.dark` color tokens but zero toggle mechanism (no `next-themes`, no provider, no toggle UI). Source: `context/changes/shadcn-design-refresh/research.md` (found 2026-08-10, user-triggered design audit). |
...
| TD-06      | no-past-lesson-scheduling | Block booking a lesson with a scheduled time in the past | yes | 🔴 Data-integrity gap flagged by user (2026-08-30): neither `Lesson.propose()` (`src/domain/lesson/Lesson.ts`) nor `createLesson.ts` (`src/app/actions/lessons/createLesson.ts:37-40`) validate `scheduledAt` against the current time — only that it's a parseable timestamp. The office UI doesn't guard this either: `WeeklyCalendar`'s week navigation lets office staff browse to any past week and `CalendarGrid`'s slot click opens the create-lesson panel unconditionally, so a lesson can be proposed and booked for a time that has already passed, with no rejection anywhere in the stack. Needs a decision on where the guard belongs (domain invariant in `Lesson.propose()` vs. UI-level slot-disabling vs. both) and how "now" is compared against `scheduled_at` given the app's UTC-based slot math. GitHub issue #68. |
