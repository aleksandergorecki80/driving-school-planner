...
- **Status:** done — all 9 phases shipped (schema/RPC foundation, old-mechanism retirement, server actions, `/lesson/[token]` page, email integration, AI-suggested rejection reasons, office resend-link button + send-to-a-different-email override, office polling, docs sync). Phase 8 (office polling) closed a confirmed gap: the "office dashboard polls every 30 seconds" outcome above was claimed since Phase 1 but never actually implemented until 2026-07-11 — `src/lib/supabase/client.ts`, built for this in `supabase-data-foundation`, remains unused by design (the shipped approach is a full `router.refresh()` on an interval, not a lighter-weight client-side subscription). Phase 7 landed in two slices: the resend-link button, then the send-to-a-different-email override (reworked mid-implementation — see PRD refs above). **Known gap found during docs sync (2026-07-11), closed 2026-09-05:** the rejection reason was persisted correctly but never displayed in the office UI. Fixed via **TD-11** (`office-rejection-reason-display`, commit `13b3082`) — `LessonRow`, `office/page.tsx`'s query, and `LessonPopover.tsx` now carry and render it. GitHub issue #51 closed. The outcome text above is now fully accurate.
...
| TD-08      | calendar-week-transition-animation | Add a subtle transition animation when navigating between weeks | done — see `plan.md` | ✅ Shipped 2026-09-05 (1 phase, commit `8e4b70f`, `context/changes/calendar-week-transition-animation/plan.md`): `CalendarGrid` is now keyed on `weekStart` and slides in from the matching side (`tw-animate-css`) on Prev/Next, respecting `prefers-reduced-motion`; keying on `weekStart` (not `lessons`) keeps the 30s `AutoRefresh` poll from replaying the animation. Mid-phase addition found during manual testing: the server round-trip gave no immediate click feedback, so Prev/Next now wrap `router.push()` in `useTransition` and disable/dim while pending. GitHub issue #72 closed. |
| TD-09      | calendar-week-jump-picker | Add a date picker to jump directly to a week from the week-range label | done — see `plan.md` | ✅ Shipped 2026-09-05 (2 phases, commits `a76ad5e`/`36c967d`, `context/changes/calendar-week-jump-picker/plan.md`): week-range label opens a shadcn Calendar+Popover (`WeekPicker.tsx`) to jump to any week; `getWeekStart()` hardened to always snap `?week=` to Monday; whole-week hover/focus highlight via pure CSS (`has-[:hover]`/`has-[:focus]`) after a React-state-driven version caused a real click-dropping regression (re-rendering the full day grid on every pointer move raced with real clicks) found and fixed during manual testing. GitHub issue #73 closed. |
| TD-10      | override-email-inline-edit | Make the lesson-link recipient email inline-editable via a pencil icon | done | ✅ Shipped 2026-09-06 (1 phase, `context/changes/override-email-inline-edit/plan.md`): `OverrideEmailField` rewritten as a self-contained pencil-to-edit control (commit on blur/Enter, cancel on Escape, inline `checkValidity()` validation, "No email on file" case handled with the same pattern); `NewLessonForm` and `LessonPopover` both lift a single `overrideEmail: string \| undefined` state through one `onOverrideChange` callback — `NewLessonForm` mirrors it into a hidden input, reusing the same hidden-input-from-state pattern already used there for `category`. New component test (`OverrideEmailField.test.tsx`, 6 cases). Merged via PR #95; GitHub issue #74 closed. |
| TD-11      | office-rejection-reason-display | Show the persisted rejection reason in the office UI | done — see `plan.md` | ✅ Shipped 2026-09-05 (1 phase, commit `13b3082`, `context/changes/office-rejection-reason-display/plan.md`): `LessonRow` now carries `rejection_reason`, the office lessons query selects it, and `LessonPopover` renders it via `DetailRow` when `status === 'rejected'` and a reason is present. Closes `test-plan.md` §3 Phase 3's remaining gap. GitHub issue #51 closed. |
| TD-12      | test-data-cleanup-sweep | Sweep orphaned test-seeded rows out of the live Supabase instance | done — manual sweep, see below | ✅ Fixed 2026-09-06: a data-consistency audit found the leak had grown well beyond the 3 rows originally reported (`test-instr-cat-*`, `test-instr-lessons-*`, `test-instr-no-email-*`) — a previously-undocumented `test-office-poll-*`/`test-office-poll-student-*` batch (11 rows, 2026-07-11, likely from `instructor-responds` Phase 8 office-polling tests) had also leaked in. Manually deleted all 17 leaked rows (9 instructors + 8 students) plus 3 referencing lessons via direct service-role queries; verified 0 `test-`-prefixed rows remain. This was the "one-off manual cleanup" option from the original report. The other option (defensive `globalSetup`/`globalTeardown` name-prefix sweep to catch future leaks automatically) was **not** implemented — recurrence is still possible if a test run is interrupted again; re-open as a fresh item if that's wanted. GitHub issue #35 closed. |
| TD-13      | cancel-lesson-single-to-maybesingle | Fix unreachable "not found" branch in `cancelLesson` | done | ✅ Shipped 2026-09-06 (PR #92, commit `0826483`): `cancelLesson` (moved to `src/app/actions/lessons/cancelLesson.ts` since this item was found) swapped `.single()` for `.maybeSingle()`, same fix already applied to `regenerateLessonToken`. New regression test covers the previously-untested unknown-lesson-id path. GitHub issue #32 closed by merge. |
| TD-14      | lesson-block-status-badge | Add a status badge/label to lesson blocks in the calendar grid | done — see `plan.md` | ✅ Shipped 2026-09-06 (1 phase, commits `a6b8e82`/`9a9be11`, PR #90, `context/changes/lesson-block-status-badge/plan.md`): `LessonBlock` now shows `LESSON_STATUS[status].label` as a shrunk shadcn `Badge`, inline with the category and right-aligned (moved off a stacked third line mid-implementation — a manual screenshot review found it left a large unused strip of horizontal space); `aria-label` now includes the status label too. New `LessonBlock.test.tsx` covers all 3 statuses. GitHub issue #81 closed. |
| TD-15      | past-lesson-timezone-check | Fix timezone bug letting past lessons slip through (TD-06 regression) | done — see `plan.md` | 🔴 Live bug reported by user (2026-09-05, screenshot): booked a lesson at 15:30 while real Warsaw local time was 17:03 — both the `CalendarGrid` click-guard and the `book_lesson` RPC let it through (a bad row was created and manually cancelled during triage). Root cause: this app stores lesson times as naive local wall-clock time labeled UTC (`formatLessonDateTime` hardcodes `timeZone: 'UTC'`), but 3 independent past-time checks (`CalendarGrid.tsx`, `Lesson.propose()`, the `book_lesson` RPC) compared this naive value against the *true* UTC instant instead of the office's real current wall-clock time — off by Poland's UTC+2 (CEST) offset. Regression in TD-06, not caused by same-day TD-08/09 work. ✅ Shipped 2026-09-06 (4 phases, commits `894a621`/`36acdb4`/`dfdc603`/`09f4bbb`, `context/changes/past-lesson-timezone-check/plan.md`): new shared `src/lib/office-time.ts` helper (`officeNowAsNaiveUTC()`, `Europe/Warsaw` hardcoded, matching this single-location app's lack of any other config); all 3 checks now use it; 3 existing tests re-mocked against the helper, 3 new regression tests reproduce the exact bug shape (unit ×2, integration ×1 against the real hosted Supabase instance); new SQL migration applied to the live project via `supabase db push`. GitHub issue #85 closed. |
