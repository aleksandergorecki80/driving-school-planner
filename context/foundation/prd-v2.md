...
- **[new]** FR-013: Office can send a lesson's notification link (FR-002) to a one-shot override email address, entered at lesson creation or resend time, instead of the instructor's own on-file address. Priority: must-have. Change: new — a narrow, explicit exception to the original PRD's "no in-app instructor profile management" non-goal; unlike the original resolution below, `instructors.email` itself stays non-editable from the office UI — the override is never persisted and applies to that one send only.
  > Socratic: resolved directly by the user, then reworked mid-implementation (2026-07-11, see `context/changes/instructor-responds/change.md`): the original resolution made instructor email a distinct, office-editable field. The shipped design instead keeps `instructors.email` read-only and adds a non-persisted, one-shot override address per send. Closes Open Question #1 below.
...
- No broader instructor profile management — the original PRD's "no in-app instructor profile management" non-goal still holds; `instructors.email` remains non-editable, and FR-013's one-shot send override is not a profile field. No profile fields become editable as part of this change.
