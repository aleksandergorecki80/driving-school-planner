...
| 3 | Status loop correctness | Prove poll returns live DB state on each cycle; prove rejection reason persisted and visible to office | #4, #5 | integration + e2e | partially unblocked — `instructor-responds` Phase 8 (2026-07-11) proved poll freshness at the integration level (`src/app/office/page.test.ts`); rejection-reason *persistence* was already proven in Phase 3 (`lessons.test.ts`), but rejection-reason *visibility to office* is a confirmed gap — never implemented in the UI (`LessonRow`, `office/page.tsx`, `LessonPopover.tsx` all omit `rejection_reason`). e2e coverage for this phase still not wired (see §2 e2e row below). | context/changes/instructor-responds/ |
