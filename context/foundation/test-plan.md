...
| 3 | Status loop correctness | Prove poll returns live DB state on each cycle; prove rejection reason persisted and visible to office | #4, #5 | integration + e2e | mostly unblocked — `instructor-responds` Phase 8 (2026-07-11) proved poll freshness at the integration level (`src/app/office/page.test.ts`); rejection-reason *persistence* was already proven in Phase 3 (`lessons.test.ts`); rejection-reason *visibility to office* was a confirmed gap, closed 2026-09-05 via `office-rejection-reason-display` (commit `13b3082`) — `LessonRow`, `office/page.tsx`, `LessonPopover.tsx` now carry and render `rejection_reason`. Remaining gap: e2e coverage for this phase still not wired (see §2 e2e row below). | context/changes/instructor-responds/, context/changes/office-rejection-reason-display/ |
